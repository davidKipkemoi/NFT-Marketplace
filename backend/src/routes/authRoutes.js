...
// Wallet Authentication - Single-step Authenticate Route
router.post('/wallet/authenticate', async (req, res) => {
  ...
  try {
    ...
    try {
      recoveredAddress = ethers.verifyMessage(SIGN_IN_MESSAGE, signature);
    } catch (sigError) {
    ...
    if (fetchProfileError && fetchProfileError.code !== 'PGRST116') {
      console.error('Error fetching profile by wallet address:', fetchProfileError.message);
      return res.status(500).json({ message: 'Error fetching profile', error: fetchProfileError.message });
    }

    if (existingProfile) {
      userIdToAuthenticate = existingProfile.id;
    } else {
      ...
      if (insertError) {
        console.error('Error creating temporary profile:', insertError.message);
        return res.status(500).json({ message: 'Error creating temporary profile', error: insertError.message });
      }
      userIdToAuthenticate = newProfile[0].id;
    }

    // Issue a custom JWT for wallet-authenticated users
    const token = jwt.sign(
      { walletAddress, profileId: userIdToAuthenticate },
      JWT_SECRET,
      { expiresIn: '7d' }
    );
    ...
    console.error('Unexpected error during wallet login:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
});
